...
`[VERIFY: 四个 scan root 下被实际索引到的 MD 文件总数]`。`[VERIFY: 上一次同步里命中的 pure-move 次数，这就是 v2 重写值不值的指标]`。已知边界讲清楚：这是单机索引器，每个 MD 文件读前 2000 个字符（`head -c`）让 LLM 给一段 takeaway。不解析代码、不跟链接走、不处理二进制附件，对哪些项目「有意思」也没意见。9 个分类是我自己的口味，套到别人的文件系统上一定不合。但它能干好我需要的那件事：把散落的 MD 变成一个新 session 能真的加载的东西。
